		...
		if (creditRatingResponse.getAcceptable()) {
			...
			for (OrderItem item : items) {
				...
				if (!productAvailabilityResponse.getAvailable()) {
					log.info("Product with ID " + item.getProductId()
							+ " has not enough capacity to fullfil the order (" + item.getAmount() + ").");
			...
			log.info("Order declined because credit rating too low (" + creditRatingResponse.getCreditRating() + ").");
